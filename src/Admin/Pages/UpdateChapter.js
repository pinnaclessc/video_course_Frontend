...
//       try {
//         console.log(`selected ChapterId:${selectedChapter}`)
//         const response = await fetch(`http://localhost:8000/chapters/${selectedChapter}/topics`);
//         if (!response.ok) {
//           throw new Error(`HTTP error! Status: ${response.status}`);
//         }
//         const data = await response.json();
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/chapters/${selectedChapter}/topics`);
        if (!response.ok) {
          console.log(`selected ChapterId if ok:${selectedChapter}`)
          throw new Error(`HTTP error! Status: ${response.status}`);
        }
        const data = await response.json();

        // Assuming the data returned is an array of topics
        setTopics(data);
