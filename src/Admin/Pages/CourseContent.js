...
const CourseComponent = ({ courseId }) => {
  // Assuming data is available in the component's state
  const data = [
  ...
  return (
    <div>
      ...
      {course.topics.map((topic) => (
        <div key={topic._id}>
          <h3>{topic.videoTitle}</h3>
          <p>Video: <a href={topic.selectedVideo} target="_blank" rel="noopener noreferrer">Watch Video</a></p>
          <p>PDF: <a href={topic.selectedPdf} target="_blank" rel="noopener noreferrer">View PDF</a></p>
        </div>
      ))}
    </div>
  );
