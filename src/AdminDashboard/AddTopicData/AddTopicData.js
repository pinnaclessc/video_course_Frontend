...
const AddTopicForm = () => {
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      ...
      try {
        const response = await fetch(`http://13.200.156.92:8000/api/chapters/course/${selectedCourse}`);
        const data = await response.json();
        setChapters(data);
  ...
  useEffect(() => {
    const fetchResources = async () => {
      if (!selectedCourse) return;
      try {
        const videosResponse = await fetch(`http://13.200.156.92:8000/api/videos/course/${selectedCourse}`);
